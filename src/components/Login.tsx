...
import { ArrowUpRight, Loader2, ShieldCheck, Lock, Mail, User } from "lucide-react";
...
const Login = () => {
    ...
    const handleEmailLogin = async (e: React.FormEvent) => {
        ...
        try {
            const nameToUse = fullName.trim() || trimmedEmail.split("@")[0];
            const userId = generateDeterministicUUID(trimmedEmail);
            const pfpUrl = `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(nameToUse)}`;
            ...
            if (syncRes.success) {
                ...
                toast.success(`Welcome back, ${nameToUse}!`);
                window.location.href = "/";
            } else {
                toast.error(syncRes.message || "Failed to sign in.");
            }
        } catch (err: any) {
            ...
            <main className="flex-grow flex flex-col justify-center items-center px-4 md:px-6 py-16 relative overflow-hidden">
                ...
                <div className="w-full max-w-md space-y-6 text-center bg-[#0d0d0d] p-8 md:p-10 rounded-3xl border border-neutral-800 shadow-2xl shadow-black/80 relative">
                    ...
                    <div className="pt-4 border-t border-neutral-800/80 flex items-center justify-center gap-2 text-xs text-neutral-500">
                        ...
                        <span>Protected by Delta Auth & JWT</span>
                    </div>
                </div>
            </main>
        </div>
